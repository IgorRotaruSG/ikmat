			...
			if (_t == 'save') {
				...
				if (go) {
					...
					if ($(th).find('.colorwill').length == 0) {
						if ($(th).find('select[data-role="slider"]').val() == 'on' && $(th).find('input[name="frequency_id"]:checked').val() == undefined) {
							go = false;
...
function loadBackStep(step) {
	...
	if (step > 1) {
		...
		db.getDbInstance('registration').get(String(db_back_step), showStepFromDB);
	}
	if (step == 1) {
		step = 2;
		c = true;
...
function newCompanyRegistrationOff(special) {
	...
	if (special && typeof special == 'boolean') {
		db.getDbInstance('registration').query(function(doc, emit) {
			if (doc.step == 1 || doc.step == 2) {
				emit(doc.id, doc.data);
			}
		}, function(err, results) {
			if (results.rows.length == 2) {
				try {
					var s1 = JSON.parse(results.rows[0].value);
					var s2 = JSON.parse(results.rows[1].value);
		...
		if (t == max_stepsB) {
			...
			html += '<button type="submit" data-theme="d" onclick="redirectHaccpPage()">' + $.t('nav.haccp') + '</button>';
			html += '</div>';
			mySwiper.appendSlide(html, 'swiper-slide');
		}
		console.info('t = ', t);
		db.getDbInstance('registration').get(String(t), function(error, results) {
				if (results) {
					try {
						var s1 = JSON.parse(results.data);
