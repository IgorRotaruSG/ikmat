...
function reportsGetCurrentDate() {
    ...
    return d.getFullYear() + '-' +
        (d.getMonth()+1).toString().replace(/\d{0,2}/,function(m){return '0'.slice(m.length-1) + m;}) + '-' +
        (lastDayOfMonth.getDate()).toString().replace(/\d{0,2}/,function(m){return '0'.slice(m.length-1) + m;});
        //(d.getDate()).toString().replace(/\d{0,2}/,function(m){return '0'.slice(m.length-1) + m;});
}

var reports_date_start = new Date().getFullYear() + '-' + (new Date().getMonth()+1).toString().replace(/\d{0,2}/,function(m){return '0'.slice(m.length-1) + m;}) + '-01';
...
//    $('#reports-select-monthly-month').off('change').on('change', function(){
    $('#reports-select-monthly-month').on('change', function(){
        ...
        var m_m = m.toString().replace(/\d{0,2}/,function(m){return '0'.slice(m.length-1) + m;});
        ...
        $('#reports-type-select').on('change', function(){
            ...
            switch (type) {
                case 'crt_year':
                    ...
                    break;
                case 'crt_month':
                    d_start = d.getFullYear() + '-' + (d.getMonth()+1).toString().replace(/\d{0,2}/,function(m){return '0'.slice(m.length-1) + m;}) + '-01';
                    d_end = reportsGetCurrentDate();
                    break;
                case 'yearly':
...
function reportsView(data) {
    ...
    $('.svgString').each(function(){
        if ( $(this).val() != '' ) {
            ...
            i.width = '150';
            $(i).appendTo($(this).parent() );// append the image (SVG) to DOM.
        }
    });
