...
;( function ( window, document, $, undefined ) {
	...
	$.swipebox = function( elem, options ) {
        ...
        //<a id="flowchart-delete" title="Remove Flowchart">Slett</a>\
		plugin.settings = {};
		...
		ui = {
			...
			hideBars : function () {
				...
				if ( this.doCssTrans() ) {
					...
					$( '#swipebox-bottom-bar' ).animate( { bottom : '-40px' }, 500 );
					setTimeout( function() {
						bars.removeClass( 'visible-bars' );
					}, 1000 );
				}
			...
			actions : function () {
                ...
                $( '#flowchart-delete' ).bind( action, function( event ) {
                    event.preventDefault();
                    event.stopPropagation();
                    $this.deleteFlowchart(event);
                    $this.setTimeout();
                } );

                $( '#flowchart-print' ).bind( action, function( event ) {
                    event.preventDefault();
                    event.stopPropagation();
                    $this.printFlowchart(event);
                    $this.setTimeout();
                } );

                $( '#flowchart-email' ).bind( action, function( event ) {
                    event.preventDefault();
                    event.stopPropagation();
                    $this.emailFlowchart(event);
                    $this.setTimeout();
                } );
			...
			setTitle : function ( index ) {
				...
				if ( title ) {
                    ...
                    /*var delete_button = ' <span class="pull-right">\
                                            <a id="flowchart-delete" title="Remove Flowchart">Slett</a>\
                                        </span>';
					$( '#swipebox-title' ).append( delete_button );*/
				} else {
					$( '#swipebox-top-bar' ).hide();
				}
			},
            ...
            /**
             * CUSTOM -> delete flowchart
             */
            deleteFlowchart: function () {
                var index = $( '#swipebox-slider .slide' ).index( $( '#swipebox-slider .slide.current' ) );
                var flowchartId = null;

                if ( elements[ index ] !== undefined ) {
                    flowchartId = elements[ index ].flowchart_id;
                }
                if ( flowchartId ) {
            ...
            /**
             * CUSTOM -> email flowchart
             */
            emailFlowchart: function () {
                var index = $( '#swipebox-slider .slide' ).index( $( '#swipebox-slider .slide.current' ) );
                var flowchartId = null;
